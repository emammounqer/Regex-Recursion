...
function permutations(n, k) {
    ...
    const stack = []
    let nNum = [...Array(n)].map((_, i) => (i + 1).toString())

    function rec(nNum, k, stack) {
        if (k === 0 && stack.length > 0) arr.push(stack.join(''))

        for (let i = 0; i < nNum.length; i++) {
            const pop = nNum.splice(i, 1)
            stack.push(pop)
